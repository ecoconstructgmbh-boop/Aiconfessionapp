import { Cross, Heart, BookOpen, Sparkles, MessageSquare } from "lucide-react";
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-center h-[calc(100vh-240px)] animate-in fade-in duration-700 overflow-hidden">
        <div className="max-w-2xl mx-auto text-center space-y-4 md:space-y-8 p-4 md:p-8 pb-20">
          {/* Divine Symbol */}
        ...
        <div className="fixed bottom-16 md:bottom-20 left-0 right-0 z-40 px-4 animate-in slide-in-from-bottom-4 duration-700 delay-500">
          <div className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto justify-center">
          {/* Feedback Button */}
          <button
            onClick={() => setShowFeedbackModal(true)}
            className="group relative px-4 md:px-6 py-3 bg-gradient-to-r from-blue-500/90 to-cyan-500/90 backdrop-blur-sm text-white rounded-full shadow-2xl hover:shadow-[0_0_30px_rgba(59,130,246,0.5)] transition-all duration-300 hover:scale-105 active:scale-95 border border-blue-400/30"
          >
            <span className="relative z-10 flex items-center gap-2 text-sm md:text-base font-medium justify-center">
              <MessageSquare className="w-4 h-4 md:w-5 md:h-5" />
              <span className="hidden md:inline">Жалобы и предложения</span>
              <span className="md:hidden">Фидбек</span>
            </span>
            <div className="absolute inset-0 bg-gradient-to-r from-blue-400/50 to-cyan-400/50 rounded-full blur-lg group-hover:blur-xl transition-all opacity-75"></div>
          </button>

          {/* Donation Button */}
          <button
            onClick={() => setShowDonationModal(true)}
            className="group relative px-4 md:px-6 py-3 bg-gradient-to-r from-red-500/90 to-pink-500/90 backdrop-blur-sm text-white rounded-full shadow-2xl hover:shadow-[0_0_30px_rgba(239,68,68,0.5)] transition-all duration-300 hover:scale-105 active:scale-95 border border-red-400/30"
          >
            <span className="relative z-10 flex items-center gap-2 text-sm md:text-base font-medium justify-center">
              <Heart className="w-4 h-4 md:w-5 md:h-5 fill-white/50 animate-pulse" />
              <span className="hidden md:inline">Поддержать проект</span>
              <span className="md:hidden">Донат</span>
            </span>
            <div className="absolute inset-0 bg-gradient-to-r from-red-400/50 to-pink-400/50 rounded-full blur-lg group-hover:blur-xl transition-all opacity-75"></div>
          </button>
          </div>
        </div>
      </div>
      ...
      {/* Feedback Modal */}
      {showFeedbackModal && (
        <FeedbackModal onClose={() => setShowFeedbackModal(false)} user={user} />
      )}
    </>
  );
}